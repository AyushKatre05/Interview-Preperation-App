...
const QuestionsSection = ({mockInterviewQuestion,activeQuestionIndex}) => {
  ...
  return (
    <div className='p-5 border rounded-lg'>
        ...
        {mockInterviewQuestion && mockInterviewQuestion.map((ques,index)=>(
            <h2 className={`p-2 bg-green-200 rounded-full cursor-pointer text-xs md:text-sm text-center ${activeQuestionIndex==index && 'bg-primary text-white'}`}>Question {index+1}</h2>
        ))}
        </div>
